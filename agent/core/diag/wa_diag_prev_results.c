...
int WA_DIAG_PREV_RESULTS_Info(void *instanceHandle, void * initHandle, json_t **params)
{
    ...
    json_t * config = NULL;
    int status = 1;
    time_t expiry_time = DEFAULT_EXPIRY_TIME; // minutes
    ...
    WA_DBG("WA_DIAG_PREV_RESULTS_Info(): results expiry time is %ld min (0=never)\n", expiry_time);

    const WA_AGG_AggregateResults_t *results = WA_AGG_GetPreviousResults();
    if (results)
    {
        if (!expiry_time || ((time(0) - results->end_time) <= (expiry_time * 60)))
        {
